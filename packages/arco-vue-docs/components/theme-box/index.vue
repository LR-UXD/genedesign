...
import { IconSkin, IconLink } from 'genedesign/es/icon';
    ...
    const fetchThemeList = async (current: number, search: string) => {
      ...
      try {
        const data = await axios.get(
          `${apiBasename}/themes/api/open/themes/list?pageSize=6&currentPage=${current}&depLibrary=genedesign&keyword=${search}`
        );
